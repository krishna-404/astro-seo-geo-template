...
- ✅ **Markdown tables auto-wrapped in `.table-scroll`** by a rehype plugin in
  ...
  phone. CI enforces the wrap on built HTML too.
...
- 🔧 **Zone dashboard settings** (no diff, no history — recorded in
  PLAYBOOK Phase 6): Full (Strict) SSL, Always Use HTTPS, zone HSTS ON,
  www→apex Redirect Rule, WAF rate-limit rule on `/api/contact`,
  Email Obfuscation OFF, Rocket Loader OFF.
- ⬜ **Custom domain attach + www handling** — per site, PLAYBOOK Phase 5/6.
...
- 🔧 **Rate limiting**: a WAF rate-limiting rule on `/api/contact`
  (dashboard; free plan includes one rule). ~10 req/min per IP.
...
- ✅ **robots.txt open, sitemap referenced.**
...
- ✅ **Playwright and sharp are NOT dependencies** — installed in CI/at
  publish time, keeping a 300MB browser out of `npm ci`.
...
- ✅ **RFC 8288 `Link: rel="describedby"` headers** pointing every response
  at `llms.txt` / `llms-full.txt`.
...
- ✅ `npm run build` = types + zod schemas + render in one step.
...
- ✅ `src/assets` = processed/hashed by the image pipeline; `public/` =
  byte-for-byte at stable URLs. Fixed-URL files (favicons, robots.txt,
  verification files, `.well-known/`) MUST be in `public/`.
...
- ✅ `.well-known/security.txt` (RFC 9116) with a real contact and an
  `Expires` — so a researcher has somewhere better than a public issue.
